...
const GeometricShapes: React.FC<GeometricShapesProps> = ({ isDarkMode }) => {
  return (
    <section className={`py-20 relative overflow-hidden ${isDarkMode ? 'bg-crypto-dark-900' : 'bg-crypto-red-200'}`}>
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-20 left-[10%] w-24 h-24 rounded-lg bg-crypto-red-400 opacity-30 rotate-12 animate-float"></div>
        <div className="absolute top-40 right-[15%] w-32 h-32 rounded-full bg-crypto-red-500 opacity-20 animate-float" style={{ animationDelay: '1s' }}></div>
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full bg-crypto-red-600 opacity-10 animate-pulse-slow"></div>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8">
            <div className={`aspect-square flex items-center justify-center ${isDarkMode ? 'bg-crypto-dark-700' : 'bg-white'} rounded-xl shadow-lg p-4 transform transition-transform hover:scale-105`}>
              <div className="relative w-24 h-24">
                <div className="absolute inset-0 bg-crypto-red-500 rounded-lg transform rotate-45"></div>
              </div>
            </div>
